...
def get_population(request):
    try:
        ...
        for feature in feature_list:
            filter_params = {key: value for key, value in feature.items() if value}
            filtered_data = Population.objects.filter(**filter_params)

            population_sum = filtered_data.aggregate(total_population=Sum('population'))['total_population'] or 0

            color, value_range = get_color(population_sum, family_id)

            feature_key = "|".join(str(filter_params.get(k, "")) for k in feature.keys())

            response_data[feature_key] = {
                **filter_params,
...
def get_data_filtering_fields(request):
    ...
    boundary_name_vs_data_filtering_fields = {
    ...
    return boundary_name_vs_data_filtering_fields[boundary]
