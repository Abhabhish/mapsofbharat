...
def get_population(request):
    try:
        ...
        color_range_family = ColorRangeFamily.objects.get(id=family_id)
        color_ranges = ColorRange.objects.filter(color_range_family=color_range_family).values()
        filtered_data = (
            Population.objects
        ...
        response_data = {'color_ranges':list(color_ranges),'filtered_data':list(filtered_data)}
